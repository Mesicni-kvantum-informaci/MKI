...
\noindent 
Pojmenujte oranžově zvýrazněná logická hradla a určete pravdivostní hodnotu signálu v místech označených otazníkem.
\begin{center}
...
\node (D) at ($(and2.output) + (0.7, 0)$) {\small{?}};
\node (?1) at ($(xor1.output) + (0.2, 0)$) {\small{?}};
\node (?2) at ($(or1.output) + (0.2, 0)$) {\small{?}};
\node (?3) at ($(not2.output) + (0.3, 0.45)$) {\small{?}};
\node (A) at (0, 0) {\small{1}};
\node (B) at (0, -1) {\small{0}};
\node (C) at (0, -2) {\small{1}};  
